...
for i, inputfile in enumerate(clamdata.inputfiles('maininput')):
    ...
    r = os.system(bindir + "frog -c " + bindir + "../share/frog/frog.cfg " + shellsafe(cmdoptions) + " -t " + shellsafe(str(inputfile),'"') + " --id=" + shellsafe(docid,"'") + " -X " + shellsafe(outputdir + outputstem + ".xml","'") + " -o " + shellsafe(outputdir + outputstem + ".frog.out","'") + " --threads=1")
    if r != 0:
...
for i, inputfile in enumerate(clamdata.inputfiles('foliainput')):
    ...
    print("Invoking Frog",file=sys.stderr)
    r = os.system(bindir + "frog -c " + bindir + "../share/frog/frog.cfg " + shellsafe(cmdoptions) + " -x " + shellsafe(str(inputfile),'"') + " -X " + shellsafe(outputdir + outputstem + ".xml","'") + " -o " + shellsafe(outputdir + outputstem + ".frog.out","'") + " --threads=1")
    if r != 0:
